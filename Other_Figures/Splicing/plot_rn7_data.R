...
#' with rn7 data. Review `MotrpacRatTraining6mo::plot_feature_normalized_data`
#' for any questions. This function is used just one time for a panel for the splicing
#' dataset. (Figure S3)
                          ...
                          ...)
{
  ...
  if (is.null(title)) {
    if (add_gene_symbol) {
      ...
      # title = paste0(sprintf("%s", gene_symbol), " Min Adj p: ", Training_DA)
    }
    else {
    ...
    if (add_gene_symbol) {
      ...
      # title = paste0(sprintf("%s (%s)", title, gene_symbol), " Min Adj p: ", Training_DA)
    }
  }
