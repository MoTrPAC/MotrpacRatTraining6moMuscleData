# Purpose: Use WGCNA to determine clusters of related features. Will combine 
# global, acet, phos, redox data types
...
# output folder
fs::dir_create("../output/WGCNA_combined_Male")
out_path_base <- "../output/WGCNA_combined_Male"

# process_data
# Load processed protein-level MSnSet
(load("../output/RD0a_1.1-msnsets_processed.RData"))
m_ls <- purrr::map(m_ls, \(x){
   sampleNames(x) <- pData(x)$pid
   pData(x)$viallabel <- NULL
   pData(x)$tmt11_channel <- NULL
   pData(x)$tmt_plex <- NULL
   return(x)
})

m3 <- purrr::reduce(m_ls, MSnbase::combine)
# dim(exprs(m3))
...
# fix col name
fData(m4)$gene_name <- fData(m4)$gene_symbol

# Remove rows with missing gene symbols
m4 <- m4[!is.na(fData(m4)$gene_name), ]
# dim(exprs(m4))

# filter to 60% completeness
m4 <- filter_by_occurrence(m4, 0.60)
...
if (FALSE) {
    ...
    png(paste0(out_path_base, "/SoftPower_plot.png"), width = 10, height = 5, units = "in", res = 100)
...
fs::dir_create(glue("../output/WGCNA_combined_Male/softPower_{softPower}"))
out_path <- paste0("../output/WGCNA_combined_Male/softPower_", softPower)
...
df.final <- cbind(fData(m4), moduleColors, exprs(m4))
# dim(df.final) 

write_xlsx(select(df.final, protein_id:moduleColors), path = glue::glue("{out_path}/RD1b_1-WGCNA_table_softpwr_{softPower}.xlsx"))
...
save(gene_clusters, file = glue::glue("{out_path}/RD1b_2-ORA_gene_clusters_sp{softPower}.RData"), compress = TRUE)
...
x@compareClusterResult %>%
  ...
  write_xlsx(path = glue::glue("{out_path}/RD1b_3-WGCNA_ORA_BP_results_sp{softPower}.xlsx"))
...
b <- 1.1
ggsave(glue::glue("{out_path}/RD1b_4-WGCNA_ORA_dotplot_pvalue_1_sp{softPower}.png"), p1,
       height = 8*b, width = 7.5*b, dpi = 150)
...
ME_final <- ME_final[sampleNames(m4),]
m_me <- MSnSet(exprs = t(as.matrix(ME_final)), pData = pData(m4))

ptype_to_test <- setdiff(varLabels(m_me), c("pid", "bid", "viallabel", "tmt11_channel", "tmt_plex",
                                            "timepoint", "exp_group", "sex"))
...
png(glue::glue("{out_path}/RD1b_5-WGCNA_module_cor_heatmap_sp{softPower}_limma.png"), 
    width = 4, 
    height = 7,
    units = "in", res = 200)
...
# get MEs with  >= 1 sig ptype
p_mat2 <- lm_df %>% 
   select(-logFC) %>% 
   pivot_wider(names_from = name, values_from = P.Value) %>% 
   column_to_rownames() %>% 
   as.matrix()
colnames(p_mat2) <- colnames(cor_mat)
rownames(p_mat2) <- rownames(cor_mat)

sig_MEs <- rownames(p_mat2)[apply(p_mat2 < 0.1, 1, \(x){any(x)})] %>% 
   str_replace("^ME", "")
...
b <- 1.1
ggsave(glue::glue("{out_path}/RD1b_6-WGCNA_ORA_dotplot_pvalue_1_sp{softPower}_sig_MEs_only.png"), p1,
       height = 9*b, width = 8.5*b, dpi = 150)
...
# reduce number of modules for plots ----

# get MEs with  >= 1 sig ptype (adjP < 0.3)
sig_MEs <- rownames(p_mat)[apply(p_mat < 0.3, 1, \(x){any(x)})] %>% 
   str_replace("^ME", "")



## remake phenotype cor heatmap ----  

# only sig modules
cor_mat_rnames <- str_replace(rownames(cor_mat), "^ME", "")
cor_mat2 <- cor_mat[cor_mat_rnames %in% sig_MEs, ]

p_mat_rnames <- str_replace(rownames(p_mat), "^ME", "")
p_mat2 <- p_mat[p_mat_rnames %in% sig_MEs, ]


# Markers for different levels of significance
comb_signif <- gtools::stars.pval(p_mat2)

anno_colors <- list(module = sub("^ME", "", rownames(cor_mat2)))
names(anno_colors$module) <- rownames(cor_mat2)
...
ra <- HeatmapAnnotation(
   "Module Size" = anno_simple(rownames(cor_mat2),
                               # border = TRUE,
                               pch = as.character(mod_count[rownames(mod_count) != "grey"]),
                               pt_gp = gpar(col = pt_color),
                               col = anno_colors$module,
...
ht <- Heatmap(cor_mat2,
              ...
              row_title_gp = gpar(fontsize = 8),
              column_title_gp = gpar(fontsize = 8), 
              width = ncol(cor_mat2)*unit(0.2, "in"),
              height = nrow(cor_mat2)*unit(0.2, "in"),
              # show_heatmap_legend = FALSE,
              row_dend_side = "right",
              row_names_side = "left",
              column_title_side = "top",
              column_dend_height = unit(0.2, "in"),
              row_dend_width = unit(0.2, "in"),
              col = circlize::colorRamp2(breaks = c(-max(abs(cor_mat2)), 0, max(abs(cor_mat2))),
                                         colors = c("blue", "white", "red")),
              column_names_gp = gpar(fontsize = 7),
              row_names_gp = gpar(fontsize = 7),
              heatmap_legend_param = list(
...
num_go_terms <- length(res2$Description)
num_modules <- length(sig_MEs)


ggsave(glue::glue("{out_path}/RD1b_6-WGCNA_ORA_dotplot_pvalue_1_sp{softPower}_sig_MEs_only.png"), p1,
       height = 8, width = 5, dpi = 300)
